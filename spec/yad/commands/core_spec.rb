...
describe Yad::Commands::Core do

  it "should build the setup command" do
    cmd = Yad::Commands::Core.build_setup_command('myapp', :umask => '02', :shared_subdirectories => %w(config))
  ...
  it "should build the update source code command" do
    cmd = Yad::Commands::Core.build_update_source_code_command('my checkout command', 'my export command', 'path/to/new/release')
  ...
  it "should build the update symlink command" do
    cmd = Yad::Commands::Core.build_update_symlink_command('path/to/current/release', 'path/to/new/release')
  ...
  it "should build the remove directory command" do
    cmd = Yad::Commands::Core.build_remove_directory_command('path/to/directory')
    cmd.should eql("rm -rf path/to/directory")
  end
  
  it "should build the revision log command" do
    cmd = Yad::Commands::Core.build_revision_log_command('timestamp', 'inline_revision_identifier_command', 'path/to/new/release', 'path/to/deployment')
